...
| `host-sync.sh` | Send the production periphery to the host's server directory (`/home/baldurs/nwneeserver` by default) | Compose file, entrypoint and host scripts, `config/host/` env files, `config/mysql-init/`, `migration/`, `cnr-editor/` and the `.mod`. Never touches haks, TLK, server vault, NWN database or logs; starts nothing. `--dry-run` lists the changes. See `documentation/repository/host-sync.md` |
