...
{
  SetLocalInt(oPC, "REZARILMATER", 1);
  DelayCommand(1440.0, DeleteLocalInt(oPC, "REZARILMATER"));
  ...
  if(iProb <= 25 + iSab)
  {
      effect eEfecto3 = EffectSavingThrowIncrease(SAVING_THROW_WILL,d4(1));
      effect eEfecto1 = EffectSkillIncrease(SKILL_HEAL,d12(1));
      ...
      DelayCommand(20.0, FloatingTextStringOnCreature("*¡Ilmater te ha bendecido!*", oPC, FALSE));
      GiveXPToCreature(oPC,100);
      return;
  }
  else
  {
      DelayCommand(20.0, FloatingTextStringOnCreature("*Tus rezos no han llegado a Ilmater.*", oPC, FALSE));
  ...
  {
  FloatingTextStringOnCreature("*Tu alineamiento no te permite rezar a Ilmater.*", oPC, FALSE);
    }
}
