...
//    }
    ...
    else if(!GetIsObjectValid(GetNearestSeenOrHeardEnemyNotDead(HENCH_MONSTER_DONT_CHECK_HEARD_MONSTER)))
    {
        ...
        // sometimes waypoints are not initialized, special code for Helmed Horror in Host Tower level 4
        else if ((GetTag(OBJECT_SELF) == "2Q6_HelmHorror") || GetWalkCondition(NW_WALK_FLAG_CONSTANT))
        {
            WalkWayPoints();
        }
        else
