void main()
{
    // Bono dote soltura
    int iBono = 0;
    if(GetHasFeat(1238, GetPCSpeaker())) iBono = 10;     // Soltura epica
    else if(GetHasFeat(1221, GetPCSpeaker())) iBono = 3; // Soltura normal

    //Variables e inicializaciones.
    string sTirada = "Montar";                                          //Tipo de tirada, este texto aparecera en la frase que dice el personaje
    int iDado = Random(20)+1;                                           // Dado que se tira, a esto hay que sumarle los rangos
    int iRangos = GetSkillRank(SKILL_RIDE, GetPCSpeaker()) + iBono;     // Rangos de la criatura que activa la bolsa de dados
