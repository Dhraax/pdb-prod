...
string SPCS_D = "SPCS_Storage";    // Name of the DB to be used to store items
...
string CofreID = GetTag(GetNearestObject(OBJECT_TYPE_PLACEABLE));
...
StoreCampaignObject(SPCS_D, CofreID, oSpawn);
AssignCommand(GetArea(OBJECT_SELF), DestroyObject(oSpawn));

// "Unlocks" the chest for others to use.
...
FloatingTextStringOnCreature("*¡" + sCheck + " objetos guardados satisfactoriamente!*", oPC);
