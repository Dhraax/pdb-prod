...
//Int de Vgzchapuza y pentaculo retocados por Uki que es mas molon:
    int iSerrin = 0;

    object oItm = GetSpellCastItem();
      if(oItm == OBJECT_INVALID && GetItemPossessedBy(OBJECT_SELF, "serrin_negro")!= OBJECT_INVALID)
      {
      iSerrin = 1;
      pentaano(GetLocation(OBJECT_SELF),1.0);
      SendMessageToPC(OBJECT_SELF,"<c&&'>¡El conjuro parece haberse potenciado con el serrín lóbrego!</c>");

      object oIngrediente = GetItemPossessedBy(OBJECT_SELF,"serrin_negro");
      DelayCommand(0.1,DestroyObject(oIngrediente));
     }
//
    ...
    effect eCurse = EffectCurse(2+iSerrin, 2+iSerrin, 2+iSerrin, 2+iSerrin, 2+iSerrin, 2+iSerrin);
