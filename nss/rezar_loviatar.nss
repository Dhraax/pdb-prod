...
if(iAli1 == ALIGNMENT_LAWFUL && iAli2 == ALIGNMENT_NEUTRAL ||
    ...
    iAli1 == ALIGNMENT_LAWFUL && iAli2 == ALIGNMENT_EVIL)
 {
  ...
  SetLocalInt(oPC, "REZARLOVIATAR", 1);
  DelayCommand(1440.0, DeleteLocalInt(oPC, "REZARLOVIATAR"));
  ...
  if(iProb <= 25 + iSab)
  {
      ...
      DelayCommand(20.0, FloatingTextStringOnCreature("*¡Loviatar esta contigo!*", oPC, FALSE));
      GiveXPToCreature(oPC,100);
      return;
  }
  else
  {
      DelayCommand(20.0, FloatingTextStringOnCreature("*Tus rezos no han llegado a Loviatar.*", oPC, FALSE));
  ...
  {
  FloatingTextStringOnCreature("*Eres un blasfemo indigno y tu alineamiento no te permite rezar a Loviatar.*", oPC, FALSE);
    }
}
