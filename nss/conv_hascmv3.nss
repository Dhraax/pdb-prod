...
int StartingConditional()
{
    ...
    while(nPos<4) {
        ...
        if (nClass != CLASS_TYPE_CLERIC && nClass != CLASS_TYPE_DRUID && nClass != CLASS_TYPE_WIZARD && nClass != CLASS_TYPE_SORCERER &&  nClass != CLASS_TYPE_BARD && nClass != CLASS_TYPE_BLACKGUARD &&
            ...
            continue;
        if(GetLevelByClass(CLASS_TYPE_BLACKGUARD, oPC)>=16)
            return TRUE;
        if(GetTotalCasterLevel(oPC, nClass) >= 16)
            return TRUE;
      }
  ...
  if(iNivelClerigo + iNivelGuardiaNegro >= 16) return TRUE;
  if(iNivelClerigo + iNivelOrcus >= 16) return TRUE;
  if((iNivelMago + iNivelMaestroLividez) >= 16) return TRUE;
  if((iNivelHechicero + iNivelMaestroLividez) >= 16) return TRUE;*/
