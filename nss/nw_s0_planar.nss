...
void BonosLigadura(object oPC = OBJECT_SELF)
{
    ...
    if(oConvocado == OBJECT_INVALID) return;
        ...
        if(iSuerte == 1)
        {
            eEfectoElegido = SupernaturalEffect(EffectLinkEffects(EffectDamageReduction(10, DAMAGE_POWER_PLUS_THREE, 75), EffectVisualEffect(VFX_DUR_PROT_STONESKIN)));
            sFrase = "<c¦ó->Ligadura: La criatura llamada tiene resistencia al daño extra (reducción 10/3+).</c>";
        }
        else if(iSuerte == 2)
        {
            eEfectoElegido = SupernaturalEffect(EffectRegenerate(2, 6.0));
            sFrase = "<c¦ó->Ligadura: La criatura llamada tiene el don de la regeneración (2pg cada asalto).</c>";
        }
        else if(iSuerte == 3)
        {
            eEfectoElegido = SupernaturalEffect(EffectDamageIncrease(DAMAGE_BONUS_2d6, DAMAGE_TYPE_PIERCING));
            sFrase = "<c¦ó->Ligadura: La criatura llamada tiene un ataque superior (+2d6 daño perforante).</c>";
        }
        else  if(iSuerte == 4)
        {
...
int Negociacion(object oCaster, object oCreature)
{
    ...
    if(GetGold(oCaster) > 999 ) {  oOfrenda = 10; oOro = 1000; }
    else if(GetGold(oCaster) > 499 ) { oOfrenda = 8; oOro = 500; }
    else if(GetGold(oCaster) > 299 ) { oOfrenda = 6; oOro = 300; }
