...
void main()
{
  ...
  if(GetLocalInt(oDead, "ENC_CLASS") > 0) fCR = fCR+2.0;
  ...
  while(oGroupMbr != OBJECT_INVALID)
  {
    ...
    if( PWFXP_CheckDistance(oDead, oGroupMbr) || oGroupMbr == oKiller)
    {
      ...
      if(nXP > 0)
      {
          if(GetLocalInt(oDead, "JEFAZO") > 0)
          {
              int iBonusJefazoXP;
              if(fCR <= 5.0) iBonusJefazoXP = 25;
              else if(fCR <= 10.0) iBonusJefazoXP = 45;
              else if(fCR <= 15.0) iBonusJefazoXP = 65;
              else if(fCR <= 20.0) iBonusJefazoXP = 85;
              else iBonusJefazoXP = 125;

              PWFXP_GiveXP(oGroupMbr, nXP + iBonusJefazoXP);
          }
          else PWFXP_GiveXP(oGroupMbr, nXP);
       }
    }
