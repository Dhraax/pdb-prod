...
void main()
{
    ...
    sSummon = "conj_ladsombras";
    object oAllegado = GetLocalObject(oPC,"DOTE_REPUTACION1_PNJ");

    // Si esta muerto, return;
    if(ObtenerIntPersistente(oPC, "LADMUERTO1") > 0 )
    {
        SendMessageToPC(oPC, "<cþ<<>Tu aliado ha caido. Visita un clerigo para traerlo de vuelta.</c>");
        IncrementRemainingFeatUses(oPC, 1284);
        return;
    }
    ...
    {
        if(ObtenerIntPersistente(oPC, "DOTE_REPUTACION1") == 1 ) //Si lo tenemos guardado
        {
            RestaurarLadron1(oPC);
            DelayCommand(1.0, GuardarLadron1(oPC));
            return;
        }
        ...
        else if(GetCanSummonOrc(oPC, sSummon)) //Doble check para evitar duplicados en la EE
        {
            ...
            GuardarIntPersistente(oPC, "DOTE_REPUTACION1", 1);
            SetLocalObject(oPC,"DOTE_REPUTACION1_PNJ",oCreature);
        }
    ...
    DelayCommand(3.0, GuardarLadron1(oPC));
 }
