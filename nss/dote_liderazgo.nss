...
void main()
{
    ...
    object oCreature;
    object oAllegado = GetLocalObject(oPC,"DOTE_LIDERAZGO_PNJ");
    ...
    {
        if(ObtenerIntPersistente(oPC, "DOTE_LIDERAZGO") == 1 ) //Si lo tenemos guardado
        {
            RestaurarLiderazgo(oPC);
            DelayCommand(1.0, GuardarLiderazgo(oPC));
            return;
        }
        ...
        else if(GetCanSummonOrc(oPC, sSummon)) //Doble check para evitar duplicados en la EE
        {
            ...
            GuardarIntPersistente(oPC, "DOTE_LIDERAZGO", 1);
            SetLocalObject(oPC,"DOTE_LIDERAZGO_PNJ",oCreature);
        }
    ...
    DelayCommand(3.0, GuardarLiderazgo(oPC));
 }
