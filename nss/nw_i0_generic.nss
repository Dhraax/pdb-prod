...
//Devuelve el druida/explorador con mayor rango de empatia animal en el grupo
//que se encuentre en el mismo area que el personaje
int FindAnimalHandleInParty(object oPC = OBJECT_SELF) {
    if(!GetIsPC(oPC))
        return -1;
    object oPartyMember = GetFirstFactionMember(oPC, TRUE);
    int nMaxHandleAnimal = -1;
    object oHandlerFound = OBJECT_INVALID;
    int nAnimalHandle;
    while(GetIsObjectValid(oPartyMember) == TRUE) {
        ...
        int nDruidLevel = GetLevelByClass(CLASS_TYPE_DRUID, oPartyMember);
        if((nDruidLevel > 0 || GetLevelByClass(CLASS_TYPE_RANGER, oPartyMember) > 0) && GetSkillRank(SKILL_ANIMAL_EMPATHY, oPartyMember, TRUE) > 0) {
            nAnimalHandle = GetSkillRank(SKILL_ANIMAL_EMPATHY, oPartyMember);
            nAnimalHandle += nDruidLevel / 2;
            if(nAnimalHandle > nMaxHandleAnimal) {
                nMaxHandleAnimal = nAnimalHandle;
            }
        }
        oPartyMember = GetNextFactionMember(oPC, TRUE);
    }
    ...
    if(GetBehaviorState(NW_FLAG_BEHAVIOR_OMNIVORE))
    {
        ...
        if(!GetIsObjectValid(oIntruder))
        {
               ...
               !GetIsObjectValid(GetAttackTarget()))
            {
                if(GetIsObjectValid(oTarget))
                {
                    ...
                    {
                        float fDistanceToTarget = GetDistanceToObject(oTarget);
                        if(fDistanceToTarget <= 100.0) {
                            //if(GetLevelByClass(CLASS_TYPE_DRUID, oTarget) == 0 && GetLevelByClass(CLASS_TYPE_RANGER, oTarget) == 0)
                            int nHandleAnimalRank = FindAnimalHandleInParty(oTarget);
                            if(nHandleAnimalRank < 0)
                            {
                                SetLocalFloat(OBJECT_SELF, "ANIMAL_MINDIST_HOSTILE", 15.0);
                                if(fDistanceToTarget<15.0) {
                                    ...
                                    object oFriend = GetNearestSeenFriend(OBJECT_SELF, 1);
                                    int nFriend = 1;
                                ...
                                int nCD = 5 + GetHitDice(OBJECT_SELF) + GetLocalInt(OBJECT_SELF, "ANIMAL_PASSIVE_DC");
                                int nResult = nHandleAnimalRank + d20() - nCD;
                                float fDistance = 3.5 - nResult / 2.0;
                                int nTime = SQLite_GetTimeStamp();
                                int nPrevTime = GetLocalInt(OBJECT_SELF, "ANIMAL_MINDIST_TIME");
                                if(nTime>nPrevTime) {
                                    SetLocalFloat(OBJECT_SELF, "ANIMAL_MINDIST_HOSTILE", fDistance<=0.0?-1.0:fDistance);
                                    //SetLocalInt(OBJECT_SELF, "ANIMAL_MINDIST_TIME", nTime+12);
                                }
                                if(GetLevelByClass(CLASS_TYPE_DRUID, oTarget) > 0 || GetLevelByClass(CLASS_TYPE_RANGER, oTarget) > 0)
                                    fDistance -= 1.5;
                                //FloatingTextStringOnCreature(GetName(OBJECT_SELF) + " - Distancia minima: " + FloatToString(GetLocalFloat(OBJECT_SELF, "ANIMAL_MINDIST_HOSTILE")), oTarget, TRUE);
        ...
        else if(!IsInConversation(OBJECT_SELF))
        {
            ...
            while(oFriend!=OBJECT_INVALID && nFriend <= 5) {
                SetLocalFloat(oFriend, "ANIMAL_MINDIST_HOSTILE", 25.0);
                SetLocalInt(oFriend, "ANIMAL_MINDIST_TIME", nTime+30);
                SetIsTemporaryEnemy(oIntruder, oFriend, FALSE, 20.0);
                nFriend++;
                oFriend = GetNearestSeenFriend(OBJECT_SELF, nFriend);
            }
        }
                    ...
                    if(!GetIsFriend(oTarget))
                    {
                        ...
                        if(fDistanceToTarget <= 100.0) {
                            ...
                            if(nHandleAnimalRank < 0)
                            {
                                ...
                                int nCD = 5 + FloatToInt(GetHitDice(OBJECT_SELF)*1.5) + GetLocalInt(OBJECT_SELF, "ANIMAL_PASSIVE_DC");
                                int nResult = nHandleAnimalRank + d20() - nCD;
                                float fDistance = 7.0 - nResult / 2.0;
                                int nTime = SQLite_GetTimeStamp();
                                int nPrevTime = GetLocalInt(OBJECT_SELF, "ANIMAL_MINDIST_TIME");
                                if(nTime>nPrevTime) {
                                    SetLocalFloat(OBJECT_SELF, "ANIMAL_MINDIST_HOSTILE", fDistance<=0.0?-1.0:fDistance);
