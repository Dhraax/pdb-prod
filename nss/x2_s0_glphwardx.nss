...
void DoDamage(int nDamage, object oTarget)
{
    ...
    effect eDam = EffectDamage(nDamage, ChangedElementalDamage(OBJECT_SELF, DAMAGE_TYPE_SONIC));
    if(nDamage > 0)
    {
...
SetLocalInt(OBJECT_SELF, "X2_L_LAST_SPELLSCHOOL_VAR", SPELL_SCHOOL_ABJURATION);
  //Declare major variables
	...
	else if (spellsIsTarget(oTarget,SPELL_TARGET_STANDARDHOSTILE,oCreator))
            {
                ...
                if (!MyResistSpell(oCreator, oTarget))
                {
                    ...
                    nDamage = GetReflexAdjustedDamage(nDamage, oTarget, (GetSpellSaveDC()+ GetChangesToSaveDC(OBJECT_SELF)), SAVING_THROW_TYPE_SONIC, oCreator);
