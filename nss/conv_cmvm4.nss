...
int StartingConditional()
{
    ...
    while(nPos<4) {
        ...
        if (nClass != CLASS_TYPE_CLERIC && nClass != CLASS_TYPE_DRUID && nClass != CLASS_TYPE_WIZARD && nClass != CLASS_TYPE_SORCERER)
            continue;
        if(GetTotalCasterLevel(oPC, nClass) >= 22)
            return TRUE;
    }
  ...
  if(iNivelClerigo >= 22) return TRUE;
  if((iNivelMago + iNivelMaestroLividez) >= 22) return TRUE;
  if((iNivelHechicero + iNivelMaestroLividez) >= 22) return TRUE;*/
