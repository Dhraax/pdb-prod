    ...
    iAli1 == ALIGNMENT_NEUTRAL && iAli2 == ALIGNMENT_GOOD) //neutral B
{
  ...
  SetLocalInt(oPC, "REZARTORM", 1);
  DelayCommand(1440.0, DeleteLocalInt(oPC, "REZARTORM"));
  ...
  if(iProb <= 25 + iSab)
  {
      ...
      DelayCommand(20.0, FloatingTextStringOnCreature("*¡Torm te ha bendecido!*", oPC, FALSE));
      GiveXPToCreature(oPC,100);
      return;
  }
  else
  {
      DelayCommand(20.0, FloatingTextStringOnCreature("*Tus rezos no han llegado a Torm.*", oPC, FALSE));
  ...
  {
  FloatingTextStringOnCreature("*Tu alineamiento no te permite rezar a Torm.*", oPC, FALSE);
    }
}
