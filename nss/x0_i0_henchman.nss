...
/**** Number of henchmen ****/
const int X0_NUMBER_HENCHMEN = 4;

const int X2_NUMBER_HENCHMEN = 4; // This won't be the same as the GetMaxHenchmen() function due to followers
...
void StoreCampaignHenchman(object oPC)
{
    ...
    for (iSlot = 1; iSlot <= iMax; iSlot++)
    {
        ...
        else
        {
            ...
            sHench = GetName(oPC) + IntToString(iSlot);
            ...
            if (!ret)
                ...
                SetLocalInt(oPC, "LOADHENCHMEN", 1 );
                SendMessageToPC(oPC, "Aliados Guardados");
        }
...
void RetrieveCampaignHenchman(object oPC)
{
    ...
    for (iSlot = 1; iSlot <= iMax; iSlot++)
    {
        ...
        if (GetIsObjectValid(oHench))
        {
            ...
            DelayCommand(0.5, DeleteLocalInt(oPC, "LOADHENCHMEN" ));
...
int AdjustXP2Levels(int nLevel, int nMin=3, int nMax=14, int nAdjust=4)
{
    ...
    for (i=1; i<= GetMaxHenchmen(); i++)
    {
        ...
        if ( GetIsObjectValid(oAssociate) )
        {
            ...
            if (GetLocalInt(oAssociate, "X2_JUST_A_FOLLOWER") == FALSE)
            {
                ...
                {
                    // * druid would have to be exposed
                    nLevel = AdjustXP2Levels(nLevel, 3, 14, 2);
                    //LevelHenchmanUpTo(oAssociate, nLevel, CLASS_TYPE_DRUID, 0, 105);
                }
                else
                if (sTag == "conj_mdl")
