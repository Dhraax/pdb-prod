...
void main()
{
  ...
  if(GetLevelByClass(CLASS_TYPE_PAL_VENGADOR,oPC)) iNivelPaladin = GetLevelByClass(CLASS_TYPE_PAL_VENGADOR,oPC);
  if(GetSpellId() != SPELL_PALADIN_SUMMON_MOUNT) return;
  ...
  if(iMontado > 0)
  {
      ...
      else
      {
          ...
          if(iAparienciaNuevaPJ == 2 || iAparienciaNuevaPJ == 3) sMontura = "cab_palponi"; // Poni de guerra
          else sMontura = "cab_palcaba"; // Caballo de guerra pesado
  ...
  if(iAparienciaPJ == 2 || iAparienciaPJ == 3) sMontura = "cab_palponi"; // Poni de guerra
  else sMontura = "cab_palcaba"; // Caballo de guerra pesado
  object oMontura = CreateObject(OBJECT_TYPE_CREATURE, sMontura, GetLocation(oPC));
