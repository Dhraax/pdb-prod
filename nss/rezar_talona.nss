...
if(iAli1 == ALIGNMENT_CHAOTIC && iAli2 == ALIGNMENT_NEUTRAL ||
    iAli1 == ALIGNMENT_NEUTRAL && iAli2 == ALIGNMENT_EVIL ||
    iAli1 == ALIGNMENT_CHAOTIC && iAli2 == ALIGNMENT_EVIL)
 {

  SetLocalInt(oPC, "REZARTALONA", 1);
  DelayCommand(1440.0, DeleteLocalInt(oPC, "REZARTALONA"));
  ...
  if(iProb <= 25 + iSab)
  {
      ...
      DelayCommand(20.0, FloatingTextStringOnCreature("*¡Talona esta contigo!*", oPC, FALSE));
      GiveXPToCreature(oPC,100);
      return;
  }
  else
  {
      DelayCommand(20.0, FloatingTextStringOnCreature("*Tus rezos no han llegado a Talona.*", oPC, FALSE));
  ...
  {
  FloatingTextStringOnCreature("*Eres un blasfemo indigno y tu alineamiento no te permite rezar a Talona.*", oPC, FALSE);
    }
}
