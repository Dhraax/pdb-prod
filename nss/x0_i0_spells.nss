...
int GetCasterAbilityModifier(object oCaster)
{
    int nClassINT = GetLevelByClass(CLASS_TYPE_WIZARD, oCaster);
    int nClassCAR1 = GetLevelByClass(CLASS_TYPE_BARD, oCaster);
    int nClassCAR2 = GetLevelByClass(CLASS_TYPE_SORCERER, oCaster);
    int nAbility;
    if ((nClassINT > 0) && (nClassINT > (nClassCAR1 || nClassCAR2)))
    {
        nAbility = ABILITY_INTELLIGENCE;
    }
    if (((nClassCAR1 || nClassCAR2) > 0) && (nClassINT < (nClassCAR1 || nClassCAR2)))
    {
...
void DoCamoflage(object oTarget)
{
    ...
    DoNoStackSkillBonus(OBJECT_SELF, oTarget, 5, SKILL_HIDE, TurnsToSeconds(nDuration),GetSpellId());
    DoNoStackSkillBonus(OBJECT_SELF, oTarget, 5, SKILL_MOVE_SILENTLY, TurnsToSeconds(nDuration),GetSpellId());
}
//::///////////////////////////////////////////////
//:: DoSpikeGrowthEffect
//:: Copyright (c) 2001 Bioware Corp.
        ...
        if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF) && (oTarget != OBJECT_SELF) && (GetObjectSeen(oTarget,OBJECT_SELF)))
        {
                ...
                if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
                {
                    for (i=1; i <= nExtraMissiles + nRemainder; i++)
                    {
                        ...
                        if(nReflexSave)
                        {
                            if(nDAMAGETYPE == DAMAGE_TYPE_MAGICAL) nDam = GetReflexAdjustedDamage(nDam, oTarget, GetSpellSaveDC(), SAVING_THROW_TYPE_SPELL);
                            else nDam = GetReflexAdjustedDamage(nDam, oTarget, GetSpellSaveDC(), SAVING_THROW_TYPE_FIRE);
                        }
...
void DoPetrification(int nPower, object oSource, object oTarget, int nSpellID, int nFortSaveDC)
{
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        if (spellsIsImmuneToPetrification(oTarget) == TRUE || PB_Race_GetIsUndead(oTarget) == TRUE)
        {
            return;
        }
        ...
        if(!MySavingThrow(SAVING_THROW_FORT, oTarget, nFortSaveDC))
        {
            ...
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oTarget, HoursToSeconds(6));
...
void RemoveSpellEffects2(int nSpell_ID, object oCaster, object oTarget, int nSpell_ID2, int nSpell_ID3)
{
    ...
    // Elimina la variable de indetectabilidad
    if (ObtenerIntPersistente(oTarget,"INDETECTABLE") == 1){ GuardarIntPersistente(oTarget,"INDETECTABLE",0);}
}
...
int spellsIsMindless(object oCreature)
{
    ...
    if (PB_Race_GetIsUndead(oCreature)) return TRUE;
...
void spellsDispelMagic(object oTarget, int nCasterLevel, effect eVis, effect eImpac, int bAll = TRUE, int bBreachSpells = FALSE)
{
    ...
    //Asyel: Sep 12 2015
    //Disipa el conjuro de indetectabilidad
...
int GZGetHighestSpellcastingClassLevel(object oCreature)
{
        ...
        for (i =1; i<= 3; i++)
        {
            ...
            if (nClass != CLASS_TYPE_INVALID)
            {
                if (nClass == CLASS_TYPE_SORCERER || nClass ==  CLASS_TYPE_WIZARD ||
                    ...
                    nClass == CLASS_TYPE_CABALLERO_ARCANO || nClass == CLASS_TYPE_SHADOW_ADEPT ||
                    nClass == CLASS_TYPE_ORCUS || nClass == CLASS_TYPE_TEURGO_MIST || nClass == CLASS_TYPE_ARCHMAGE)
                {
    ...
    //Llamamos al script de modificacion disipacion de conjuros
    nMax = nMax + ExecuteScriptAndReturnInt("add_spell_dispel", oCreature);
    return nMax;
}
...
void spellsDispelAoE(object oTargetAoE, object oCaster, int nCasterLevel)
{
    ...
    if ( nId != SPELL_LESSER_DISPEL )
    {
       ...
       if (ObtenerIntPersistente(oTargetAoE,"INDETECTABLE") == 1){ GuardarIntPersistente(oTargetAoE,"INDETECTABLE",0);}
    }
