...
void TallarObjetosOrfebreria(object oJugador, string sFraseFabricando,
                             ...
                             string sContenedor = "contenedor_engar")
{
  ...
  if(iUsosPunzon == 0)
  {
      SetLocalInt(oPunzon, "USOS", 30 + d6(3));
  }
  else if(iUsosPunzon == 1)
  {
      DelayCommand(6.0, AssignCommand(oJugador, PlayAnimation(ANIMATION_LOOPING_GET_MID, 1.0, 1.5)));
      DelayCommand(6.0, FloatingTextStringOnCreature("*¡Se te ha roto el punzon de diamante!*", oJugador));
  ...
  if(iUsosKit == 0)
  {
      SetLocalInt(oKit, "USOS", 35 + d6(4));
  }
  else if(iUsosKit == 1)
  {
      DelayCommand(6.0, FloatingTextStringOnCreature("*¡Las herramientas de tallar se han roto!*", oJugador));
  ...
  if(iLesion <= 3)
  {
      ...
      {
          int iHP = GetCurrentHitPoints(oJugador);
          effect e1 = EffectAbilityDecrease(ABILITY_DEXTERITY,4);
          ...
          DelayCommand(6.0, FloatingTextStringOnCreature("*¡Te has cortado con un cristal de gema!*", oJugador, FALSE));
          DelayCommand(6.0, PlayVoiceChat(VOICE_CHAT_PAIN1, oJugador));
      }
  ...
  int iNivelHabilidad = ObtenerIntPersistente(oJugador, "NIVELTALLADOR");
  ...
  if(iTiradaAcceso + iBonoTiradaAcceso >= 30)
  {
      ...
      if(IntToFloat(iTiradaExito) <= (fProbabilidadExito + IntToFloat(GetSkillRank(22,oJugador))))
      {
          ...
          SetLocalInt(oObjetoFabricado, "OROVENTAT", iOroVenta + iNivelHabilidad);
/*          int id3 = d2();
          if (id3 == 2)
          {*/
          CreateItemOnObject("polvo_" + GetStringRight(GetTag(oObjetoFabricado),3));
//          }
          ...
          if((iNivelHabilidad < 100) && (IntToFloat(iTiradaAprendizaje) <= fProbablidadAprendizaje))
          {
              ...
              DelayCommand(7.0, FloatingTextStringOnCreature("¡Has subido al nivel " + IntToString(iNivelHabilidad + 1) + " en Tallador!", oJugador));
              DelayCommand(7.0, SetXP(oJugador, GetXP(oJugador) + iExperiencia));
              GuardarIntPersistente(oJugador, "NIVELTALLADOR", iNivelHabilidad + 1);
          }
      ...
      else
      {
          ...
          DelayCommand(6.0, FloatingTextStringOnCreature("*¡Fracasaste al tallar el objeto!*", oJugador, FALSE));
          EliminarVariablesBanco();
          return;
      }
  }
