...
/*
    Suenyo profundo.
    Como dormir pero aumentado a 10 DG.
*/
//:://////////////////////////////////////////////
//:: Created By: Monti
...
// End of Spell Cast Hook
    ...
    int nMax = 15;// maximun hd creature affected
    int nLow;
    int nDuration = GetTotalCasterLevel(OBJECT_SELF);
    int nScaledDuration;
    nDuration = 3 + nDuration;

    ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_LOS_NORMAL_10), GetSpellTargetLocation());
    ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(949), GetSpellTargetLocation());
    string sSpellLocal = "BIOWARE_SPELL_LOCAL_SLEEP_" + ObjectToString(OBJECT_SELF);
    //Enter Metamagic conditions
    if (nMetaMagic == METAMAGIC_MAXIMIZE)
    {
        nHD = 14;//Damage is at max
    }
    ...
    while ((nHD > 0) && (bContinueLoop))
    {
        ...
        while (GetIsObjectValid(oTarget))
        {
            ...
            if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF)
                && GetRacialType(oTarget) != RACIAL_TYPE_CONSTRUCT && !PB_Race_GetIsUndead(oTarget))
            {
