...
void BonosLigadura(object oPC = OBJECT_SELF)
{
    ...
    if(oConvocado == OBJECT_INVALID) return;
        ...
        if(iSuerte == 1)
        {
            eEfectoElegido = SupernaturalEffect(EffectLinkEffects(EffectDamageReduction(10, DAMAGE_POWER_PLUS_ONE, 50), EffectVisualEffect(VFX_DUR_PROT_STONESKIN)));
            sFrase = "<c¦ó->Ligadura: La criatura llamada tiene resistencia al daño extra (reducción 10/1+).</c>";
        }
        else if(iSuerte == 2)
        {
            eEfectoElegido = SupernaturalEffect(EffectRegenerate(1, 6.0));
            sFrase = "<c¦ó->Ligadura: La criatura llamada tiene el don de la regeneración (1pg cada asalto).</c>";
        }
        else if(iSuerte == 3)
        {
            eEfectoElegido = SupernaturalEffect(EffectDamageIncrease(DAMAGE_BONUS_2d4, DAMAGE_TYPE_PIERCING));
            sFrase = "<c¦ó->Ligadura: La criatura llamada tiene un ataque superior (+2d4 daño perforante).</c>";
        }
        else  if(iSuerte == 4)
        {
...
int Negociacion(object oCaster, object oCreature)
{
    ...
    //Si eres Soldado de la luz, exito
    if(GetLevelByClass(51, oCaster)) return TRUE;

    int oOfrenda, oOro;
    //Comprobamos si tenemos oro
    if(GetGold(oCaster) > 499 ) {  oOfrenda = 10; oOro = 500; }
    else if(GetGold(oCaster) > 299 ) { oOfrenda = 8; oOro = 300; }
    else if(GetGold(oCaster) > 99 ) { oOfrenda = 6; oOro = 100; }
...
void CreateObjectVoid(int nObjectType, string sTemplate, location lLoc, object oPC = OBJECT_SELF)
{
            ...
            case ALIGNMENT_EVIL:
                {
                    ...
                    switch(d6())
                     {
                      case 1: eApariencia = 1050; break;
                      case 2: eApariencia = 1051; break;
                      case 3: eApariencia = 105; break;
                      case 4: eApariencia = 104; break;
                      case 5: eApariencia = 1046; break;
                      case 6: eApariencia = 105; break;
                     }
            ...
            case ALIGNMENT_GOOD:
                {
                    ...
                    if(GetHasSpellEffect(SPELL_PROTECTION_FROM_GOOD, oPC)      ||
                     ...
                     switch(d4())
                     {
                      case 1: eApariencia = 2950; break;
                      case 2: eApariencia = 55; break;
                      case 3: eApariencia = 2276; break;
                      case 4: eApariencia = 103; break;
                     }
                }
            break;
            case ALIGNMENT_NEUTRAL:
                {
                    if(GetHasSpellEffect(SPELL_PROTECTION_FROM_GOOD, oPC)      ||
                     ...
                     switch(d4())
                     {
                      case 1: eApariencia = 111; break;
                      case 2: eApariencia = 4009; break;
                      case 3: eApariencia = 151; break;
                      case 4: eApariencia = 155; break;
                     }
                }
            break;
        }
    ...
    switch(d20(2))
  {
    ...
    //Asignamos valores a la criatura para subir de nivel y apariencia
       ...
       DelayCommand(1.0, LevelHenchmanUpTo(oCreature, 9, nClass, 20, 86, nPack));
    ...
    //Comienza la Negociacion
     if(GetLevelByClass(51, oPC)) SetLocalInt(oPC, "CIRCULOTRUE", TRUE);
