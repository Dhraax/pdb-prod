...
void CristalAntimagia(object oPC)
{
  ...
  while(GetIsEffectValid(eBad))
  {
      if((GetEffectType(eBad) == EFFECT_TYPE_ABILITY_INCREASE ||
         ...
         (GetEffectSubType(eBad) != SUBTYPE_SUPERNATURAL && GetEffectSubType(eBad) != SUBTYPE_UNYIELDING))
  ...
  DelayCommand(1.0, ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_PWSTUN), oPC)); //VFX_FNF_PWSTUN (este es el anterior), despues tuvo el 899
}
