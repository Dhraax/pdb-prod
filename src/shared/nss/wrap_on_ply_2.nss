//::////////////////////////////////////////////////////////////////////////////
//:: Nombre del guion:  wrap_on_ply_lvl                                   //:://
//::////////////////////////////////////////////////////////////////////////////
...
void main()
{
  ...
  if(sSubraza == "drow" && GetHitDice(oPC) >= 11 &&
  ...
  //Dote Ambidiestro y Combate con Dos armas
  if(ObtenerIntPersistente(oPC, "AMB_DOSARMAS") == iNivel && GetHasFeat(1, oPC) && GetHasFeat(41, oPC)){
        BajarNivelQuedandoseA1XP(oPC);
        NWNX_Creature_RemoveFeat(oPC, 1);
        NWNX_Creature_RemoveFeat(oPC, 41);
        SendMessageToPC(oPC, "<cþ<<>¡Se te han borrado las dotes de combate con dos armas y ambidiestro por la resubida de nivel!</c>");
        return;
    }
  else if(GetHasFeat(1, oPC) && !GetHasFeat(41, oPC)) {
        NWNX_Creature_AddFeat(oPC, 41);
        GuardarIntPersistente(oPC, "AMB_DOSARMAS", iNivel);
        SendMessageToPC(oPC, "<c´þd>¡Has adquirido la dote Combate con dos armas gratuitamente!</c>");
        }
  else if(!GetHasFeat(1, oPC) && GetHasFeat(41, oPC)) {
        NWNX_Creature_AddFeat(oPC, 1);
        GuardarIntPersistente(oPC, "AMB_DOSARMAS", iNivel);
 ...
 //3.1 Alas Alma Predilecta
  if(GetLevelByClass(59, oPC) == 17)
  {
    ...
    //Si se es menos de nivel 3 de Artífice y se tiene alguno de los items, los borramos.
    if(GetLevelByClass(CLASS_TYPE_INGENIERO, oPC) < 3)
    {
        //Artillero.
        ...
        if(ObtenerIntPersistente(oPC,"CLS_ING_TIPO") == 0 && GetLevelByClass(CLASS_TYPE_INGENIERO, oPC) > 0)
        {
            BajarNivelQuedandoseA1XP(oPC);
            SendMessageToPC(oPC, "<cþ<<>Debes elegir un tipo de Artífice.</c>");
            return;
        }
        //Control de Dotes por si se modifica el tipo de Artifice a nivel 1.
        if(ObtenerIntPersistente(oPC,"CLS_ING_TIPO") == 1)
        {
            if(GetHasFeat(1769,oPC)) NWNX_Creature_RemoveFeat(oPC,1769);
            if(GetHasFeat(1207,oPC)) NWNX_Creature_RemoveFeat(oPC,1207);
            if(GetHasFeat(1770,oPC)) NWNX_Creature_RemoveFeat(oPC,1770);
            if(GetHasFeat(945,oPC)) NWNX_Creature_RemoveFeat(oPC,945);
            if(GetHasFeat(946,oPC)) NWNX_Creature_RemoveFeat(oPC,946);
        }
        else if(ObtenerIntPersistente(oPC,"CLS_ING_TIPO") == 2)
        {
            if(GetHasFeat(1768,oPC)) NWNX_Creature_RemoveFeat(oPC,1768);
            if(GetHasFeat(1770,oPC)) NWNX_Creature_RemoveFeat(oPC,1770);
            if(GetHasFeat(945,oPC)) NWNX_Creature_RemoveFeat(oPC,945);
            if(GetHasFeat(946,oPC)) NWNX_Creature_RemoveFeat(oPC,946);
        }
        else if(ObtenerIntPersistente(oPC,"CLS_ING_TIPO") == 4)
        {
            if(GetHasFeat(1769,oPC)) NWNX_Creature_RemoveFeat(oPC,1769);
            if(GetHasFeat(1206,oPC)) NWNX_Creature_RemoveFeat(oPC,1206);
            if(GetHasFeat(1207,oPC)) NWNX_Creature_RemoveFeat(oPC,1207);
            if(GetHasFeat(1768,oPC)) NWNX_Creature_RemoveFeat(oPC,1768);
        }
    }
