...
void main()
{
    ...
    object oCatapulta = OBJECT_SELF;
    if (iINT >= 10)
    {
        if (GetLocalInt(oCatapulta, "fd") == 0)
        {
            FloatingTextStringOnCreature("¡FUEGO!", oPC, TRUE);
...
/////////////////////////////////////////////////////////////////
            ...
            ActionCastSpellAtLocation(SPELL_FIREBALL, lImpact, METAMAGIC_MAXIMIZE, TRUE, PROJECTILE_PATH_TYPE_BALLISTIC);
            ...
            SetLocalInt(oCatapulta, "fd", 1);
            DelayCommand(5.0, SetLocalInt(oCatapulta, "fd", 0));
        }
        if (GetLocalInt(oCatapulta, "fd") == 1)
        {
            SendMessageToPC(oPC, "¡Espere a que se cargue de nuevo!");
        }
        if ((GetLocalInt(oCatapulta, "fd") != 0)&&(GetLocalInt(oCatapulta, "fd") != 1))
        {
            SetLocalInt(oCatapulta, "fd", 0);
        }
    }
    if (iINT < 10)
    {
        FloatingTextStringOnCreature("No puedes utilizar esta arma porque eres demasiado estúpido para entender cómo funciona.", oPC, FALSE);
        DelayCommand(0.7, FloatingTextStringOnCreature("¡Idiota!, *Se te incendia el pelo*", oPC, FALSE));
        DelayCommand(0.9, ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_FIREBALL), oPC));
        DelayCommand(0.9, ActionCastSpellAtObject(SPELL_FIREBALL, oPC, METAMAGIC_ANY, TRUE, 5, PROJECTILE_PATH_TYPE_BALLISTIC));
        DelayCommand(0.9, ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_SCREEN_BUMP), oPC));
