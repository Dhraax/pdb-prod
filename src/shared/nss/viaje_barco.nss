...
void main()
{
    ...
    int sNum = -1;
    string sWaypoint;
    if(!GetLocalInt(GetObjectByTag("viaje_barco1"), "OCUPADA")){oWaypoint = GetWaypointByTag("WP_barco1"); sNum = 1; sWaypoint = "WP_barco1";}
    else if(!GetLocalInt(GetObjectByTag("viaje_barco2"), "OCUPADA")){oWaypoint = GetWaypointByTag("WP_barco2");sNum = 2; sWaypoint = "WP_barco2";}
    else if(!GetLocalInt(GetObjectByTag("viaje_barco3"), "OCUPADA")){oWaypoint = GetWaypointByTag("WP_barco3");sNum = 3; sWaypoint = "WP_barco3";}
    else if(!GetLocalInt(GetObjectByTag("viaje_barco4"), "OCUPADA")){oWaypoint = GetWaypointByTag("WP_barco4");sNum = 4; sWaypoint = "WP_barco4";}
    //else if(!GetLocalInt(GetObjectByTag("viaje_barco5"), "OCUPADA")){oWaypoint = GetWaypointByTag("WP_barco5");sNum = 5; sWaypoint = "WP_barco5";}
    else
    {
        FloatingTextStringOnCreature("<cþ<<>Lo sentimos, pero los 5 barcos que tiene habilitado el servidor están ocupados. Inténtalo más tarde.</c>", oPC);
        return;
    }
    ...
    //Ajustes del viaje
    int nTileSet = 4; //En barco
    int nEncounterFrequency = 20; //Probabilidad de encuentro
    int nTime = 240; //Duración del viaje en segundos
    ...
    object oArea = GetObjectByTag("viaje_barco"+SP_Num);
