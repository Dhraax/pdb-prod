...
void main()
{
    ...
    sSummon = "ow_sum_sham";
    object oAllegado = GetLocalObject(oPC,"WAR_SHAM_PNJ");

    // Si esta muerto, return;
    if(ObtenerIntPersistente(oPC, "SHAMMUERTO") > 0 )
    {
    ...
    {
        if(ObtenerIntPersistente(oPC, "WAR_SHAM") == 1 ) //Si lo tenemos guardado
        {
            RestaurarOrcSham(oPC);
            DelayCommand(3.0, GuardarOrcSham(oPC));
            return;
        }
        ...
        else if(GetCanSummonOrc(oPC, sSummon)) //Doble check para evitar duplicados en la EE
        {
            ...
            GuardarIntPersistente(oPC, "WAR_SHAM", 1);
            SetLocalObject(oPC,"WAR_SHAM_PNJ",oCreature);
            ...
            if(sSubraza == "trasgo" || sSubraza == "gran trasgo")
            {
                ...
                SetCreatureAppearanceType(oCreature, APPEARANCE_TYPE_GOBLIN_SHAMAN_A);
            }
            else if(sSubraza == "ogro" || sSubraza == "ogro hechicero")
            {
                NWNX_Creature_SetRacialType(oCreature, RACIAL_TYPE_GIANT);
                SetCreatureAppearanceType(oCreature, APPEARANCE_TYPE_OGRE_MAGE);
            }
            else if(sSubraza == "gnoll")
            {
                NWNX_Creature_SetRacialType(oCreature, RACIAL_TYPE_HUMANOID_MONSTROUS);
                SetCreatureAppearanceType(oCreature, APPEARANCE_TYPE_GNOLL_WIZ);
            }
            else if(sSubraza == "minotauro")
            {
                NWNX_Creature_SetRacialType(oCreature, RACIAL_TYPE_HUMANOID_MONSTROUS);
                SetCreatureAppearanceType(oCreature, APPEARANCE_TYPE_MINOTAUR_SHAMAN);
            }
            else if(sSubraza == "kobold")
            {
                NWNX_Creature_SetRacialType(oCreature, RACIAL_TYPE_HUMANOID_REPTILIAN);
                SetCreatureAppearanceType(oCreature, APPEARANCE_TYPE_KOBOLD_SHAMAN_A);
            }
        }
    }
