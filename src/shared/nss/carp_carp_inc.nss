...
void CrearObjetosCarpinteria(object oJugador, string sFraseFabricando, string sObjetoContenedor, string sExito, int iDificultad, int iOroVenta, string sContenedor = "contenedor_banco");
...
void DestruirIngredientes()
{
  ...
  {
      if(GetStringLeft(GetTag(oIngrediente), 11) == "carptablon_" ||
         GetStringLeft(GetTag(oIngrediente), 8) == "carpacc_" ||
         GetStringLeft(GetTag(oIngrediente), 8) == "carpac2_"||
         GetTag(oIngrediente) == "brisaSusurrante") DestroyObject(oIngrediente);
...
void CrearObjetosCarpinteria(object oJugador, string sFraseFabricando,
                             ...
                             string sContenedor = "contenedor_yunque")
{
  ...
  {
      DelayCommand(6.0, FloatingTextStringOnCreature("*¡Las herramientas de carpintería se han roto!*", oJugador));
  ...
  if(iLesion <= 3)
  {
      ...
      if(iLesion == 1)
      {
          ...
          DelayCommand(6.0, FloatingTextStringOnCreature("*¡Los tablones tenían termitas! Han quedado inservibles*", oJugador, FALSE));
      }
      ...
      else
      {
          ...
          DelayCommand(6.0, FloatingTextStringOnCreature("*¡Se te clavan varias astillas en las manos!*", oJugador, FALSE));
          DelayCommand(6.0, PlayVoiceChat(VOICE_CHAT_PAIN1, oJugador));
      }
  ...
  int iNivelHabilidad = ObtenerIntPersistente(oJugador, "NIVELCARPINTERIA");
  ...
  if(iTiradaAcceso + iBonoTiradaAcceso >= 30)
  {
      ...
      if(IntToFloat(iTiradaExito) <= (fProbabilidadExito + IntToFloat(GetSkillRank(22,oJugador))) )
      {
          ...
          if((iNivelHabilidad < 100) && (IntToFloat(iTiradaAprendizaje) <= fProbablidadAprendizaje))
          {
              ...
              DelayCommand(7.0, FloatingTextStringOnCreature("¡Has subido al nivel " + IntToString(iNivelHabilidad + 1) + " en Carpintería!", oJugador));
              DelayCommand(7.0, SetXP(oJugador, GetXP(oJugador) + iExperiencia));
              GuardarIntPersistente(oJugador, "NIVELCARPINTERIA", iNivelHabilidad + 1);
          }
