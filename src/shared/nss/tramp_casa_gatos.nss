...
location lLugar = GetLocation(GetWaypointByTag("ir_clokas_puerto"));
...
if(iVariable1 != 1 || iVariable2 != 1)
    ...
    if((GetAbilityScore(oPJ, ABILITY_STRENGTH) >= 30))
   {
    ...
    DelayCommand(1.0, AssignCommand(oPJ, PlayAnimation(ANIMATION_LOOPING_GET_LOW, 1.0, 5.0)));
    DelayCommand(2.0, FloatingTextStringOnCreature("*¡¡¡Tiras fuerte de la trampilla!!!*", oPJ));
    DelayCommand(6.0, FloatingTextStringOnCreature("*¡¡¡ÉXITO!!!*", oPJ));
    DelayCommand(7.5, FloatingTextStringOnCreature("*Tiras con todas tus fuerzas de la pesada trampilla y consigues levantarla", oPJ));
    ...
    DelayCommand(60.0, DeleteLocalInt(oTrampilla_gatos, "TRAMPILLA_CASA_GATOS"));
    ...
    else
        {
         ...
         DelayCommand(1.0, FloatingTextStringOnCreature("*¡¡¡Tiras fuerte de la trampilla!!!*", oPJ));
