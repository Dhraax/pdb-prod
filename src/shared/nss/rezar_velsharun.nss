...
if(iAli1 == ALIGNMENT_LAWFUL && iAli2 == ALIGNMENT_NEUTRAL ||  //legal neutral
    ...
    iAli1 == ALIGNMENT_CHAOTIC && iAli2 == ALIGNMENT_NEUTRAL)   //caotico neutral x
{
  ...
  SetLocalInt(oPC, "REZARVELSHA", 1);
  DelayCommand(1440.0, DeleteLocalInt(oPC, "REZARVELSHA"));
  ...
  if(iProb <= 25 + iSab)
  {
      ...
      effect eEfecto4 = EffectSavingThrowIncrease(SAVING_THROW_FORT,d4(1),SAVING_THROW_TYPE_DISEASE);
      effect eEfecto3 = EffectSavingThrowIncrease(SAVING_THROW_FORT,d4(1),SAVING_THROW_TYPE_DEATH);
      ...
      DelayCommand(20.0, FloatingTextStringOnCreature("*¡Velsharun te ha bendecido!*", oPC, FALSE));
      GiveXPToCreature(oPC,100);
      return;
  }
  else
  {
      DelayCommand(20.0, FloatingTextStringOnCreature("*Tus rezos no han llegado a Velsharun.*", oPC, FALSE));
  ...
  {
  FloatingTextStringOnCreature("*Tu alineamiento no te permite rezar a Velsharun.*", oPC, FALSE);
    }
}
