void main()
{
  object oPC = GetLastDisturbed();
  object oObjeto = GetInventoryDisturbItem();

  if(GetLocalInt(oObjeto, "OROVENTA") > 0)
  {
      if(GetBaseItemType(oObjeto) == BASE_ITEM_BOLT ||
         GetBaseItemType(oObjeto) == BASE_ITEM_BULLET)
      {
          if(GetItemStackSize(oObjeto) == 99)
          {
              int iOroVenta = GetLocalInt(oObjeto, "OROVENTA");
              GiveGoldToCreature(oPC, iOroVenta);
              FloatingTextStringOnCreature("*Has vendido el objeto por " + IntToString(iOroVenta) + " po*", oPC, FALSE);
              DestroyObject(oObjeto);
              return;
          }
          else
          {
              CopyObject(oObjeto, GetLocation(OBJECT_SELF), oPC);
              DestroyObject(oObjeto);
              FloatingTextStringOnCreature("¡Las balas y virotes se venden de 99 en 99!", oPC, FALSE);
          }
      }

      else if(GetBaseItemType(oObjeto) == BASE_ITEM_THROWINGAXE ||
              GetBaseItemType(oObjeto) == BASE_ITEM_SHURIKEN ||
              GetBaseItemType(oObjeto) == BASE_ITEM_DART)
      {
          if(GetItemStackSize(oObjeto) == 50)
          {
              int iOroVenta = GetLocalInt(oObjeto, "OROVENTA");
              GiveGoldToCreature(oPC, iOroVenta);
              FloatingTextStringOnCreature("*Has vendido el objeto por " + IntToString(iOroVenta) + " po*", oPC, FALSE);
              DestroyObject(oObjeto);
              return;
          }
          else
          {
              CopyObject(oObjeto, GetLocation(OBJECT_SELF), oPC);
              DestroyObject(oObjeto);
              FloatingTextStringOnCreature("¡Las hachas arrojadizas, dardos y shurikens se venden de 50 en 50!", oPC, FALSE);
          }
      }

      else
      {
          int iOroVenta = GetLocalInt(oObjeto, "OROVENTA");
          GiveGoldToCreature(oPC, iOroVenta);
          FloatingTextStringOnCreature("*Has vendido el objeto por " + IntToString(iOroVenta) + " po*", oPC, FALSE);
          DestroyObject(oObjeto);
          return;
      }
  }

  else
  {
      CopyObject(oObjeto, GetLocation(OBJECT_SELF), oPC);
      DestroyObject(oObjeto);
      FloatingTextStringOnCreature("¡No puedes vender este objeto!", oPC, FALSE);
  }
}
