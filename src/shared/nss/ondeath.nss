...
void main()
{
    ...
    // QUEST DE LA TORRE DE AMNAGUA
    object oJugador = GetLastKiller();
    string sArea = GetTag(GetArea(oJugador));

    while(GetIsObjectValid(GetMaster(oJugador)))
    {
        oJugador = GetMaster(oJugador);
    }

    int nInt = GetLocalInt(oJugador, "NIVELTORRE");
    nInt += 1;
    object oParty = GetFirstFactionMember(oJugador);
    while (GetIsObjectValid(oParty))
    {
        if(sArea == GetTag(GetArea(oParty))) SetLocalInt(oParty, "NIVELTORRE", nInt);
        oParty = GetNextFactionMember(oJugador);
