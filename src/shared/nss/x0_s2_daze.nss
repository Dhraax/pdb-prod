...
void main()
{
    ...
    if(spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
    {
       ...
       {
            int nShadowDancerLevels = GetLevelByClass(CLASS_TYPE_SHADOWDANCER, OBJECT_SELF);
            int nCharismaMod = GetAbilityModifier(ABILITY_CHARISMA);
            //Make Will Save to negate effect
            if (!/*Will Save*/ MySavingThrow(SAVING_THROW_WILL, oTarget, 10+nShadowDancerLevels+nCharismaMod, SAVING_THROW_TYPE_MIND_SPELLS))
            {
