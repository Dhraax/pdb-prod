...
void main()
{
    ...
    sSummon = "ow_sum_fght";
    object oAllegado = GetLocalObject(oPC,"WAR_FGHT_PNJ");

    // Si esta muerto, return;
    if(ObtenerIntPersistente(oPC, "FGHTMUERTO") > 0 )
    {
    ...
    {
        if(ObtenerIntPersistente(oPC, "WAR_FGHT") == 1 ) //Si lo tenemos guardado
        {
            RestaurarOrcFght(oPC);
            DelayCommand(3.0, GuardarOrcFght(oPC));
            return;
        }
        ...
        else if(GetCanSummonOrc(oPC, sSummon)) //Doble check para evitar duplicados en la EE
        {
            ...
            GuardarIntPersistente(oPC, "WAR_FGHT", 1);
            SetLocalObject(oPC,"WAR_FGHT_PNJ",oCreature);
            ...
            if(sSubraza == "trasgo" || sSubraza == "gran trasgo")
            {
                ...
                SetCreatureAppearanceType(oCreature, APPEARANCE_TYPE_GOBLIN_CHIEF_A);
            }
            else if(sSubraza == "ogro" || sSubraza == "ogro hechicero")
            {
                NWNX_Creature_SetRacialType(oCreature, RACIAL_TYPE_GIANT);
                SetCreatureAppearanceType(oCreature, APPEARANCE_TYPE_OGRE);
            }
            else if(sSubraza == "gnoll" || sSubraza == "minotauro")
            {
                NWNX_Creature_SetRacialType(oCreature, RACIAL_TYPE_HUMANOID_MONSTROUS);
                SetCreatureAppearanceType(oCreature, APPEARANCE_TYPE_GNOLL_WARRIOR);
            }
            else if(sSubraza == "kobold")
            {
                NWNX_Creature_SetRacialType(oCreature, RACIAL_TYPE_HUMANOID_REPTILIAN);
                SetCreatureAppearanceType(oCreature, APPEARANCE_TYPE_KOBOLD_A);
            }
        }
    }
    else
