...
int StartingConditional()
{
    ...
    while(nPos<4) {
        ...
        if (nClass != CLASS_TYPE_CLERIC && nClass != CLASS_TYPE_DRUID && nClass != CLASS_TYPE_WIZARD && nClass != CLASS_TYPE_SORCERER &&  nClass != CLASS_TYPE_BARD &&  nClass != CLASS_TYPE_WARLOCK)
            continue;
        if(GetTotalCasterLevel(oPC, nClass) >= 10)
            return TRUE;
    }
  ...
  if(iNivelClerigo >= 10) return TRUE;
  if((iNivelMago + iNivelMaestroLividez) >= 10) return TRUE;
  if((iNivelHechicero + iNivelMaestroLividez) >= 10) return TRUE;
  if((iNivelBardo + iNivelMaestroLividez) >= 10) return TRUE;*/
