...
void main()
{
    ...
    sSummon = "ow_sum_barb";
    object oAllegado = GetLocalObject(oPC,"WAR_BARB_PNJ");

    // Si esta muerto, return;
    if(ObtenerIntPersistente(oPC, "BARBMUERTO") > 0 )
    {
    ...
    {
        if(ObtenerIntPersistente(oPC, "WAR_BARB") == 1 ) //Si lo tenemos guardado
        {
            RestaurarOrcBarb(oPC);
            DelayCommand(3.0, GuardarOrcBarb(oPC));
            return;
        }
        ...
        else if(GetCanSummonOrc(oPC, sSummon)) //Doble check para evitar duplicados en la EE
        {
            ...
            GuardarIntPersistente(oPC, "WAR_BARB", 1);
            SetLocalObject(oPC,"WAR_BARB_PNJ",oCreature);
