    ...
    iAli1 == ALIGNMENT_NEUTRAL && iAli2 == ALIGNMENT_GOOD)    //neutral bueno
{
  ...
  SetLocalInt(oPC, "REZARSHARESS", 1);
  DelayCommand(1440.0, DeleteLocalInt(oPC, "REZARSHARESS"));
  ...
  if(iProb <= 25 + iSab)
  {
      ...
      effect eEfecto4 = EffectDamageShield(1,d4(1),DAMAGE_TYPE_DIVINE);
      ...
      DelayCommand(20.0, FloatingTextStringOnCreature("*¡Sharess te ha bendecido!*", oPC, FALSE));
      GiveXPToCreature(oPC,100);
      return;
  }
  else
  {
      DelayCommand(20.0, FloatingTextStringOnCreature("*Tus rezos no han llegado a Sharess.*", oPC, FALSE));
  ...
  {
  FloatingTextStringOnCreature("*Tu alineamiento no te permite rezar a Sharess.*", oPC, FALSE);
    }
}
