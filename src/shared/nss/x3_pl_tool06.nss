...
FloatingTextStringOnCreature("Reagrupando..", oOriginalMaster, FALSE);
    ...
    while (GetIsObjectValid(oMinion))
    {
        ...
        if (IsMinion(oMinion,oOriginalMaster))
        {
            ...
            if (!CheckConversation(oMinion, GetCurrentAction(oMinion)))
            {
                ...
                AssignCommand(oMinion, ActionMoveToObject(oOriginalMaster));
            }
        }
...
void AttackAI(object oTarget, int nCommand, float fTracker, object oArea, object oOriginalMaster)
{
    ...
    {
        FloatingTextStringOnCreature("DEBUG 1", oOriginalMaster, FALSE);
...
{
//SendMessageToPC(oOriginalMaster,"(PB)Convocados:: Ataque individual");
...
void TryToMoveToLocation(location lTarget, int nCommand,object oOriginalMaster,float fCloseEnough)
{
    ...
    if(!iOverrideMove)DelayCommand(0.5, TryToMoveToLocation(lTarget, nCommand,oOriginalMaster,fCloseEnough));
}
...
{
//SendMessageToPC(oOriginalMaster,"(PB)Convocados: Movimiento individual");
    if(GetIsObjectValid(oHench))
    {
...
{
//SendMessageToPC(oOriginalMaster,"(PB)Convocados: Ataque en grupo");
    // Going to loop through members of the party
    object oMinion = GetFirstFactionMember(oOriginalMaster, FALSE);
    while (GetIsObjectValid(oMinion))
    {
...
{
    // SendMessageToPC(oOriginalMaster,"(PB)Convocados: Moviendo al grupo");
    // Going to loop through members of the party
    object oMinion = GetFirstFactionMember(oOriginalMaster, FALSE);
    while (GetIsObjectValid(oMinion))
    {
...
void TryToMoveToCreature(object oTarget, int nCommand, object oArea,object oOriginalMaster,float fCloseEnough)
{
    ...
    if  (CheckConversation(OBJECT_SELF, nAction) || IsInConversation(oOriginalMaster))
    {
        FloatingTextStringOnCreature("DEBUG 3", oOriginalMaster, FALSE);
        SetLocalInt(OBJECT_SELF, "bmc_active", 0);
        RegroupParty(oOriginalMaster);
        return;
    }
...
void CheckLockStatus(object oTarget,object oOriginalMaster, int nCommand, object oArea)
{
    ...
    if  (CheckConversation(OBJECT_SELF, nAction) || IsInConversation(oOriginalMaster))
    {
        FloatingTextStringOnCreature("DEBUG 4", oOriginalMaster, FALSE);
        SetLocalInt(OBJECT_SELF, "bmc_active", 0);
        RegroupParty(oOriginalMaster);
        return;
    }
...
void SingleUnlock(object oTarget,object oOriginalMaster,object oArea)
{
    ...
    {
        FloatingTextStringOnCreature(GetName(OBJECT_SELF) + " no puede desbloquear la cerradura.", GetMaster(), FALSE);
        ResetHenchmenState();
        SetLocalInt(OBJECT_SELF, "bmc_active", 0);
        return;
    }
...
void GroupUnlock(object oTarget,object oOriginalMaster,object oArea)
{
    ...
    while (GetIsObjectValid(oMinion))
    {
        ...
        if (GetMaster(oMinion) == oOriginalMaster)
        {
            ...
            {
                FloatingTextStringOnCreature("Enviando" + GetName(oMinion) + " a desbloquear la cerradura.", oOriginalMaster, FALSE);
...
void CheckDisarmStatus(object oTarget,object oOriginalMaster, int nCommand, int nDisarmActive, object oArea,int iControl)
{
   int Trap_OFF =GetLocalInt(OBJECT_SELF,"Hen_TrapOff");
    ...
    if  (CheckConversation(OBJECT_SELF, nAction) || IsInConversation(oOriginalMaster))
    {
        //FloatingTextStringOnCreature("DEBUG 5", oOriginalMaster, FALSE);
        SetLocalInt(OBJECT_SELF, "bmc_active", 0);
        RegroupParty(oOriginalMaster);
        return;
    }
    ...
    if (GetTrapDetectedBy(oTarget, OBJECT_SELF))
    {
        ...
        else
        {
            if (!nDisarmActive)
            {
                ...
                {
                    FloatingTextStringOnCreature(GetName(OBJECT_SELF) + " no puede desactivar la trampa.", GetMaster(), FALSE);
                    ResetHenchmenState();
                    SetLocalInt(OBJECT_SELF, "bmc_active", 0);
                    return;
                }
            ...
            else if (nAction == ACTION_INVALID && nDisarmActive)
            {
                ...
                if(iControl && Trap_OFF)SetAssociateState(NW_ASC_DISARM_TRAPS,TRUE,OBJECT_SELF);
                ClearAllActions();
                bkAttemptToDisarmTrap(oTarget, TRUE);
                if(Trap_OFF && iControl)SetAssociateState(NW_ASC_DISARM_TRAPS,FALSE,OBJECT_SELF);
            }
        }
...
void GroupDisarm(object oTarget,object oOriginalMaster,int iControl)
{
    ...
    while (GetIsObjectValid(oMinion))
    {
        ...
        if (GetMaster(oMinion) == oOriginalMaster)
        {
           ...
           {
                FloatingTextStringOnCreature("Enviando " + GetName(oMinion) + " a desactivar la trampa.", oOriginalMaster, FALSE);
...
{
FloatingTextStringOnCreature("Debug Attack", oMaster, FALSE);
object oN_Enemy =GetNearestEnemy(oHench);
float fEnemy =GetDistanceBetween(oHench,oN_Enemy);
if(fEnemy<=8.0)AssignCommand(oHench,IndAttack(oMaster,oHench,oN_Enemy));
}
...
void HenchWithDoors(object oOriginalMaster,object oHench,object oTarget,int iControl,float fCloseEnough,int iStayAfterAction)
{
  ...
  // Disarm trapped doors
            ...
            if (GetTrapDetectedBy(oTarget, oOriginalMaster)&& iControl)
            {
            FloatingTextStringOnCreature("Enviando " + GetName(oHench) + " a desactivar la trampa.", oOriginalMaster, FALSE);
           ...
           if(GetLocked(oTarget)&& iControl)
            {
              FloatingTextStringOnCreature("Enviando " + GetName(oHench) + " a forzar la cerradura.", oOriginalMaster, FALSE);
              AssignCommand(oHench, SingleUnlock(oTarget,oOriginalMaster,oArea));
            return;
            }
           else if(iControl && GetIsOpen(oTarget)==FALSE)
           {
          FloatingTextStringOnCreature("Parece que no esta cerrado. Enviando "+GetName(oHench)+" a abrir", oOriginalMaster, FALSE);
          ...
          return;
           ...
           {
          FloatingTextStringOnCreature("Ya esta abierto. Enviando "+GetName(oHench)+" a cerrarlo", oOriginalMaster, FALSE);
          AssignCommand(oHench, SingleMoveToLocation(GetLocation(oHench),oOriginalMaster,fCloseEnough));
          AssignCommand(oHench,ActionMoveToObject(oTarget,TRUE,1.0));
...
//so each henchmen say something differant.
FloatingTextStringOnCreature("Lo siento, No se puede identificar "+sT_Name+" ", oHench, TRUE);
return;
}
...
//You could use local string for this responds
//so each henchmen say something differant.
FloatingTextStringOnCreature("Todo listo", oHench, TRUE);
...
FloatingTextStringOnCreature("Enviado "+GetName(oHench)+" a recoger "+sT_Name, oOriginalMaster, FALSE);
AssignCommand(oHench, SingleMoveToLocation(GetLocation(oHench),oOriginalMaster,fCloseEnough));
...
AssignCommand(oHench, SingleUnlock(oTarget,oOriginalMaster,oArea));
           ...
           {
           FloatingTextStringOnCreature("Este objeto no esta cerrado ni tiene trampa", oOriginalMaster, FALSE);
           AssignCommand(oHench, SingleMoveToLocation(GetLocation(oHench),oOriginalMaster,fCloseEnough));
           AssignCommand(oHench,ActionMoveToObject(oTarget,TRUE,1.0));
...
//object oPer =GetPersistentItemOnPc(oCaster);
   ...
   object oOriginalMaster = oCaster;
    ...
    {
        FloatingTextStringOnCreature("En conversacion, reagrupando grupo.", oOriginalMaster, FALSE);
        RegroupParty(oOriginalMaster);
        return;
    }
    ...
    SendMessageToPC(oOriginalMaster,"(PB)Convocados: Ahora controlas a todo el grupo");
    return;
    }
    ...
    SendMessageToPC(oOriginalMaster,"(PB)Convocados: Controlas a "+GetName(oTarget));
    ...
    if (GetIsObjectValid(oTarget))
    {
        ...
        // if (GetObjectType(oTarget) == OBJECT_TYPE_ITEM)//DISABLED BY DHRAAX
        // {
