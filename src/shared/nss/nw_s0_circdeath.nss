...
// End of Spell Cast Hook
    ...
    int nHD = d4(GetTotalCasterLevel(OBJECT_SELF)); //Roll to see how many HD worth of creature will be killed
    ...
    {
        nHD = 4 * (GetTotalCasterLevel(OBJECT_SELF));//Damage is at max
    }
    ...
    while ((nHD > 0) && (bContinueLoop))
    {
        ...
        if(bContinueLoop == TRUE)
        {
            ...
            if(!MyResistSpell(OBJECT_SELF, oLowest, fDelay))
            {
                ...
                {
                    if(GetHasSpellEffect(1329,oLowest))
                     {
                     SetImmortal(oLowest, FALSE);
                     }
