...
void main()
{
    ...
    float fScale = GetLocalFloat(OBJECT_SELF, "ESCALADO");
    if (fScale > 0.0f) {
    ...
    */
    ...
    //GoLoT: Comportamiento de animales. Si no se les define la variable
    //ANIMAL_BEHAVIOR se comportan automaticamente como omnivoros.
    //Si se define a 1, se comportan como herbivoros (Huyen de todos salvo druidas/exploradores)
    //Si se define a 3, se comportan como carnivoros (Atacan a todos)
    //Si se definen a cualquier otro valor, omnivoros (Atacan a todos salvo druidas/exploradores)
    //El comportamiento por defecto para cada tipo se puede modificar en
    //la funcion DetermineSpecialBehavior() de nw_i0_generic.nss
    int nBehavior = GetLocalInt(OBJECT_SELF, "ANIMAL_BEHAVIOR");
    if(nBehavior != 0) {
        SetBehaviorState(NW_FLAG_BEHAVIOR_SPECIAL);
        SetBehaviorState(NW_FLAG_BEHAVIOR_CARNIVORE, FALSE);
        SetBehaviorState(NW_FLAG_BEHAVIOR_HERBIVORE, FALSE);
        SetBehaviorState(NW_FLAG_BEHAVIOR_OMNIVORE, FALSE);
        SetSpawnInCondition(NW_FLAG_HEARTBEAT_EVENT);
        switch(nBehavior) {
            case 1:
    ...
    switch (GetRacialType(OBJECT_SELF)) {
        case RACIAL_TYPE_MAGICAL_BEAST:
            SetLocalInt(OBJECT_SELF, "ANIMAL_PASSIVE_DC", GetLocalInt(OBJECT_SELF, "ANIMAL_PASSIVE_DC")+5);
        case RACIAL_TYPE_BEAST:
            if(!GetBehaviorState(NW_FLAG_BEHAVIOR_OMNIVORE) && !GetBehaviorState(NW_FLAG_BEHAVIOR_HERBIVORE))
                SetBehaviorState(NW_FLAG_BEHAVIOR_CARNIVORE);
        case RACIAL_TYPE_ANIMAL:
            if(!GetBehaviorState(NW_FLAG_BEHAVIOR_CARNIVORE) && !GetBehaviorState(NW_FLAG_BEHAVIOR_HERBIVORE))
