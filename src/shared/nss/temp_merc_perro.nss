...
string sEsclavo = "temp_merc_perro";
...
if(GetHenchman(oPC) != OBJECT_INVALID)
    {
     FloatingTextStringOnCreature("*¡Solo puedes contratar a 1 mercenario!*", oPC, FALSE);
     return;
    }
...
if(GetGold(oPC) < iDinero)
    {
     FloatingTextStringOnCreature("*¡No tienes suficiente dinero para contratar los servicios de este mercenario!*", oPC, FALSE);
     return;
    }
...
SetLocalString(oEsclavo, "AMO", GetName(oPC));
}
