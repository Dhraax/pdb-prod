        ...
        while(GetIsObjectValid(oTarget))
        {
            ...
            if ( nTargetSize >= CREATURE_SIZE_MEDIUM )
            {
                if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, GetAreaOfEffectCreator()))
                {
                    ...
                    if (nTargetSize == CREATURE_SIZE_TINY )
                    {
                        nOppossedGrappleCheckModifiers = nOppossedGrappleCheckModifiers - 8;
                    }
                    else if (nTargetSize == CREATURE_SIZE_SMALL )
                    {
                        nOppossedGrappleCheckModifiers = nOppossedGrappleCheckModifiers - 4;
                    }
                    else if (nTargetSize == CREATURE_SIZE_MEDIUM )
                    {
                        nOppossedGrappleCheckModifiers = nOppossedGrappleCheckModifiers + 0;
                    }
                    else if (nTargetSize == CREATURE_SIZE_LARGE )
                    {
                        nOppossedGrappleCheckModifiers = nOppossedGrappleCheckModifiers + 4;
                    }
