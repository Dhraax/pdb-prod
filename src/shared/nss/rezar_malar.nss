...
if(iAli1 ==  ALIGNMENT_CHAOTIC && iAli2 == ALIGNMENT_NEUTRAL ||
    ...
    iAli1 == ALIGNMENT_CHAOTIC && iAli2 == ALIGNMENT_EVIL)
 {
  ...
  SetLocalInt(oPC, "REZARMALAR", 1);
  DelayCommand(1440.0, DeleteLocalInt(oPC, "REZARMALAR"));
  ...
  if(iProb <= 25 + iSab)
  {
      ...
      DelayCommand(20.0, FloatingTextStringOnCreature("*¡Malar esta contigo!*", oPC, FALSE));
      GiveXPToCreature(oPC,100);
      return;
  }
  else
  {
      DelayCommand(20.0, FloatingTextStringOnCreature("*Tus rezos no han llegado a Malar.*", oPC, FALSE));
  ...
  {
  FloatingTextStringOnCreature("*Eres un blasfemo indigno y tu alineamiento no te permite rezar a Malar.*", oPC, FALSE);
    }
}
