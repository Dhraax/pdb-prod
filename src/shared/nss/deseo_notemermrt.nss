...
void main()
{
  ...
  int iDeseoNoTemerMuerte = ObtenerIntPersistente(oPC, "DESEO_NOTEMERMUERTE");
  if(iDeseoNoTemerMuerte == 1)
  {
  ...
  AssignCommand(OBJECT_SELF, SpeakString("¡Entonces reduciré la capacidad de tus preocupaciones! ¡Ni lo pensarás!"));

  GuardarIntPersistente(oPC, "DESEO_NOTEMERMUERTE", 1);
  ...
  effect eInteligencia = ExtraordinaryEffect(EffectAbilityDecrease(ABILITY_INTELLIGENCE, 12));
  DelayCommand(2.6, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eInteligencia, oPC, 3600.0));
