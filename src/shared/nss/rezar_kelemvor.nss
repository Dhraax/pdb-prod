...
if(iAli1 == ALIGNMENT_LAWFUL && iAli2 == ALIGNMENT_NEUTRAL ||  //legal N
    ...
    iAli1 == ALIGNMENT_LAWFUL && iAli2 == ALIGNMENT_EVIL)   //legal maloso
{

  SetLocalInt(oPC, "REZARKELEM", 1);
  DelayCommand(1440.0, DeleteLocalInt(oPC, "REZARKELEM"));
  ...
  if(iProb <= 25 + iSab)
  {
      effect eEfecto1 = EffectDamageShield(1,d4(1),DAMAGE_TYPE_DIVINE);
      effect eEfecto2 = EffectVisualEffect(VFX_FNF_STRIKE_HOLY);
      ...
      DelayCommand(20.0, FloatingTextStringOnCreature("*¡Kélemvor te ha bendecido!*", oPC, FALSE));
      GiveXPToCreature(oPC,100);
      return;
  }
  else
  {
      DelayCommand(20.0, FloatingTextStringOnCreature("*Tus rezos no han llegado a Kélemvor.*", oPC, FALSE));
  ...
  {
  FloatingTextStringOnCreature("*Tu alineamiento no te permite rezar a Kélemvor.*", oPC, FALSE);
    }
}
