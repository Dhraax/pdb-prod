...
void main()
{
    ...
    {
        SendMessageToPC(oPC, "Hay otra persona jugando, por favor espera tu turno.");
        return;
    }
    if(GetGold(oPC) < BASECOST)
    {
        SendMessageToPC(oPC, "No puedes permitirte la apuesta.");
        return;
    }
    nT=1;
    ...
    SendMessageToPC(oPC,"Lanzas los dados, uno a uno...");
...
void GivePayout(int nMult)
{
    ...
    if(nChk < nPayout)
    {
        SendMessageToPC(oPC,"¡Justo entre medias! Ganas a la Casa.");
        nPayout=nChk;
    }
    else
        SubHouse(nPayout);
    SendMessageToPC(oPC,"Has ganado "+IntToString(nPayout)+" de oro.");
...
void DetermineResults()
{
    ...
    SendMessageToPC(oPC, "Pierdes, más suerte la próxima vez.");
    AddHouse(BASECOST);
