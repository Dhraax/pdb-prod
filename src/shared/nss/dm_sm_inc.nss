...
{  int iCurrentSets = GetLocalInt(oWidget, "iCurrentSets");
   ...
   while( iLoop < iCounter)
   {
      ...
      int DMAPAPOS = FindSubString(GetName(oProp), "DMAPA_");
      int DMAESC = FindSubString(GetName(oProp), "DMESC_");
      string sNombre = GetName(oProp);
      //Tiene una apa cambiada por DM.
      if(DMAPAPOS != -1)
      {
        int nStart = FindSubString(sNombre, "DMAPA_") + 6;
        int nEnd = GetStringLength(sNombre);
        if(DMAESC != -1)
        {
           nEnd = FindSubString(sNombre, "DMESC_");
        }
        string sResult = GetSubString(sNombre, nStart, nEnd - nStart);
        //SendMessageToPC(GetFirstPC(), "Texto extraído: " + sResult);
        NWNX_Object_SetAppearance(oProp, StringToInt(sResult));
      }
      if(DMAESC != -1)
      {
        int nStart = FindSubString(sNombre, "DMESC_") + 6;
        string sResult = GetSubString(sNombre, nStart, GetStringLength(sNombre) - nStart);
        //SendMessageToPC(GetFirstPC(), "Texto extraído: " + sResult);
        SetObjectVisualTransform(oProp, OBJECT_VISUAL_TRANSFORM_SCALE, StringToFloat(sResult));
      }
