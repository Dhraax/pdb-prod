    ...
    */
    ...
    //--PHENOTYPE BITS-------------------------------------------------------
      ...
      {
            fNewApp = GetObjectVisualTransform(oPC, OBJECT_VISUAL_TRANSFORM_SCALE) + 0.01;
            // SendMessageToPC(oPC, "Indice Corporal: " + FloatToString(fNewApp));
            fMax = PB_Race_TamanoMaximo (oPC);
            if(fNewApp > fMax)
            {
                fNewApp = fMax;
                SendMessageToPC(oPC, "Alcanzada altura máxima.");
            }

            SetObjectVisualTransform(oPC, OBJECT_VISUAL_TRANSFORM_SCALE,fNewApp);
            GuardarIntPersistente(oPC, "CAB_ALTURA",TRUE);
            GuardarFloatPersistente(oPC, "IND_ALTURA", fNewApp);
            sMsg=FALSE;
    ...
    //--EVERYBODY/THING SECTION ---------------------------------------------
        if (fNewApp!=0.0)
        {
            /*string sEntero = IntToString(FloatToInt(fNewApp));
            string sDecimal = GetSubString(FloatToString(fNewApp), 2, 2);
            if (sDecimal!="")
            {
                //SendMessageToPC(oPC, "Decimales: " + FloatToString(fNewApp));
                SendMessageToPC(oPC, "Nuevo Indice Altura: " + sEntero + "." + sDecimal);
            }*/
            SendMessageToPC(oPC, "Nuevo índice de tamaño: " + FloatToString(fNewApp,18,3));
        }

        if (sMsg==TRUE) { SendMessageToPC(oPC, "Nueva apariencia: " + IntToString(iNewApp)); }
    }
