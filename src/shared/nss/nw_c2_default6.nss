...
void main()
{
    ...
    if(GetFleeToExit()) {
		...
		else if (!GetIsFighting(OBJECT_SELF))
		{
			...
			else
			{
                if(!GetObjectSeen(oDamager) && GetArea(OBJECT_SELF) == GetArea(oDamager))
				   {
                    // Si no veo al atacante, lo busco
                    ActionMoveToLocation(GetLocation(oDamager), TRUE);
					ActionDoCommand(HenchDetermineCombatRound(oDamager));
					}
				else
				{
		...
		else
		{
            ...
            if (!GetIsObjectValid(oTarget) || oTarget != oDamager && GetTotalDamageDealt() > GetMaxHitPoints(OBJECT_SELF) / 4 || GetHitDice(oDamager) - 2 > GetHitDice(oTarget) || Espada == "espadaplanar"
            || Espada2 == "espadaplanar")
            {
                // Switch targets
                HenchDetermineCombatRound(oDamager);
            }
    ...
    if(GetSpawnInCondition(NW_FLAG_DAMAGED_EVENT))
    {
        SignalEvent(OBJECT_SELF, EventUserDefined(EVENT_DAMAGED));
        	...
        	//Ejecutamos los poderes especiales si los tenemos
	       if(GetLocalInt(OBJECT_SELF, "PODER_ESPECIAL") > 0 && GetLocalInt(OBJECT_SELF, "YATENGOPODER") != TRUE) {
		      ExecuteScript("enc_boss", OBJECT_SELF);
        		}
}
