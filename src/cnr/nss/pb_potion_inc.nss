/// ----------------------------------------------------------------------------
/// @system  CNR alchemy
/// @file    pb_potion_inc.nss
/// @author  Dhraax
/// @brief   Activates the retained sute_her potion effects.
/// modified by: Dhraax
/// ----------------------------------------------------------------------------
...
#include "nostack_inc"
#include "inc_spells"
#include "inc_effect_ids"
#include "inc_timelock"
#include "lib_race"

/// PROD adaptation, 2026-09-12: this module still runs the effect helpers of
/// `inc_spells`, not the `inc_effects` layer this file was written against.
/// Two differences, both mechanical: the effect tag is set with TagEffect on
/// the link instead of travelling as a fifth argument to gsSPApplyEffect, and
/// FX_SUBTYPE_SAFE is written as FALSE, which is the bForceRemove this
/// gsSPRemoveEffect takes and skips Supernatural and Unyielding exactly the
/// same way. Revert both when inc_effects is ported.

// -----------------------------------------------------------------------------
//                              Function Prototypes
// -----------------------------------------------------------------------------

/// @brief Activates a retained sute_her potion by its item tag.
/// @param oPC The player character activating the potion.
/// @param sPocion The potion item tag, including the effect variant suffix.
...
void usarPocionHerboristeria(object oPC, string sPocion)
{
    // Four alchemy outputs are food or drink rather than potions: Pure Water,
    // the FoodRICH cookie, the FoodNORM roots and the DrinkHIGH juice. None of
    // them owns an entry in the effect switch below, and the id their tag
    // parses to - 002 for the roots, 127 and 128 for the other two - belongs
    // to no potion. Returning here keeps a later case 2 from firing off a meal.
    if (sPocion == "sute_her_DM1"
        || sPocion == "sute_her_128_075_n_FoodRICH"
        || sPocion == "sute_her_002_005_n_FoodNORM"
        || sPocion == "sute_her_127_075_n_DrinkHIGH")
    {
        return;
    }
    ...
    // The cooldown is a timelock now, not a numeric local named after the
    // potion id. The name is read off the item as it is drunk, and it is what
    // every message about this cooldown says - the refusal, the floating text
    // and the "available again" line - the same way the drinks in
    // pb_mod_activate name theirs. "La misma pocion" told the drinker nothing
    // he did not already know; which one is the part he was missing.
    //
    // Drinking early still costs the potion and still hurts: the lock only
    // tells the player when it is safe, it does not refuse the sip.
    string sNombre = GetName(GetItemActivated());
    if (sNombre == "")
    {
        sNombre = sPocion;
    }

    if (GetIsTimelocked(oPC, sNombre)){
        // The floating text says it hurts; this says for how much longer,
        // which is what the drinker actually needs. Same red line the other
        // timelocked abilities give - Ocultarse a Simple Vista has always
        // shown it.
        TimelockErrorMessage(oPC, sNombre);
        FloatingTextStringOnCreature("*¡Tomar " + sNombre + " otra vez tan seguido te produce un intenso dolor!*", oPC, FALSE);
    ...
    switch (idPocion) {
        case 3:
            ...
            SetTimelock(oPC, iDuracionNormal * 60, sNombre, 0, 0);
            break;
        case 4:
            //Ferrea, 7/3
            ...
            gsSPRemoveEffect(oPC, SPELL_INVALID, OBJECT_INVALID, FX_ID_IRONGUTS, FALSE);
            gsSPApplyEffect(oPC, TagEffect(eLink, FX_ID_IRONGUTS), SPELL_INVALID, TurnsToSeconds(iDuracionNormal));
        ...
        case 6:
            ...
            SetTimelock(oPC, iDuracionRestringida * 60, sNombre, 0, 0);
            break;
        case 7:
            //Radiante, 4
        ...
        case 9:
            ...
            gsSPRemoveEffect(oPC, SPELL_INVALID, OBJECT_INVALID, FX_ID_ENDURE_ELEM, FALSE);
            gsSPApplyEffect(oPC, TagEffect(eLink, FX_ID_ENDURE_ELEM), SPELL_INVALID, TurnsToSeconds(iDuracionNormal));
            ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oPC);
        ...
        case 21:
            ...
            if(PB_Race_GetIsUndead(oPC)) {
                gsSPRemoveEffect(oPC, SPELL_INVALID, OBJECT_INVALID, FX_ID_STONE_BONES, FALSE);
                gsSPApplyEffect(oPC, TagEffect(eLink, FX_ID_STONE_BONES), SPELL_INVALID, TurnsToSeconds(iDuracionNormal));
                ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oPC);
            } else {
        ...
        case 45:
            //Agitada 4/4
            // Was: refuse when SPELL_HASTE is present. That guarded one of the
            // five sources of this benefit and only in one direction - the
            // potion saw a running haste and declined, while a haste cast after
            // the potion stacked on top of it, and the two other potions were
            // invisible to it either way. It replaces the identity now, like
            // every other source of it.
            ...
            gsSPRemoveEffect(oPC, SPELL_INVALID, OBJECT_INVALID, FX_ID_SPEED_CIDER, FALSE);
            gsSPRemoveEffect(oPC, SPELL_INVALID, OBJECT_INVALID, FX_ID_SPEED, FALSE);
            gsSPRemoveEffect(oPC, SPELL_INVALID, OBJECT_INVALID, FX_ID_SPEED_VFX, FALSE);

            gsSPApplyEffect(oPC, TagEffect(eLink, FX_ID_SPEED), SPELL_INVALID, TurnsToSeconds(iDuracionRestringida));
        ...
        case 55:
            //Reparadora 5/3/1
                ...
                gsSPRemoveEffect(oPC, SPELL_INVALID, OBJECT_INVALID, FX_ID_MONSTROUS_REGEN, FALSE);
                gsSPApplyEffect(oPC, TagEffect(eLink, FX_ID_MONSTROUS_REGEN), SPELL_INVALID, TurnsToSeconds(iDuracionRestringida));
                ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oPC);
        ...
        case 61:
            ...
            ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oPC);
            gsSPRemoveEffect(oPC, SPELL_INVALID, OBJECT_INVALID, FX_ID_SHIELD_SPELL, FALSE);
            gsSPApplyEffect(oPC, TagEffect(eLink, FX_ID_SHIELD_SPELL), SPELL_INVALID, TurnsToSeconds(iDuracionNormal));
        ...
        case 67:
            ...
            gsSPRemoveEffect(oPC, SPELL_INVALID, OBJECT_INVALID, FX_ID_MAGE_ARMOR, FALSE);
            gsSPApplyEffect(oPC, TagEffect(eLink, FX_ID_MAGE_ARMOR), SPELL_INVALID, TurnsToSeconds(iDuracionNormal));
            ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oPC);
        ...
        case 72:
            ...
            ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis2, oPC);
            gsSPRemoveEffect(oPC, SPELL_INVALID, OBJECT_INVALID, FX_ID_STONESKIN, FALSE);
            gsSPApplyEffect(oPC, TagEffect(eLink, FX_ID_STONESKIN), SPELL_INVALID, TurnsToSeconds(iDuracionNormal));
        ...
        case 74:
            ...
            gsSPRemoveEffect(oPC, SPELL_INVALID, OBJECT_INVALID, FX_ID_PROT_ELEMENTS, FALSE);
            gsSPApplyEffect(oPC, TagEffect(eLink, FX_ID_PROT_ELEMENTS), SPELL_INVALID, TurnsToSeconds(iDuracionNormal));
            ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oPC);
        ...
        case 94:
            ...
            gsSPRemoveEffect(oPC, SPELL_INVALID, OBJECT_INVALID, FX_ID_SHIELD_MAGIC, FALSE);

            gsSPApplyEffect(oPC, TagEffect(eLink, FX_ID_SHIELD_MAGIC), SPELL_INVALID, TurnsToSeconds(iDuracionRestringida));
        ...
        case 97:
            ...
            eLink = EffectLinkEffects(eApp, eDur);
            eLink = EffectLinkEffects(eLink, eVis);
            gsSPRemoveEffect(oPC, SPELL_INVALID, OBJECT_INVALID, FX_ID_SHIELD_ACID, FALSE);

            gsSPApplyEffect(oPC, TagEffect(eLink, FX_ID_SHIELD_ACID), SPELL_INVALID, TurnsToSeconds(iDuracionRestringida));
        ...
        case 98:
            ...
            gsSPApplyEffect(oPC, TagEffect(eLink, FX_ID_SPEED), SPELL_INVALID, TurnsToSeconds(iDuracionRestringida));
            ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oPC);
            gsSPApplyEffect(oPC, TagEffect(eVis2, FX_ID_SPEED_VFX), SPELL_INVALID, TurnsToSeconds(iDuracionRestringida));
...
gsSPRemoveEffect(oPC, SPELL_INVALID, OBJECT_INVALID, FX_ID_SHIELD_FIRE, FALSE);

            gsSPApplyEffect(oPC, TagEffect(eLink, FX_ID_SHIELD_FIRE), SPELL_INVALID, TurnsToSeconds(iDuracionRestringida));
        ...
        case 109:
            ...
            RemoveEffectsFromSpell(oPC, SPELL_WOUNDING_WHISPERS);
            gsSPRemoveEffect(oPC, SPELL_INVALID, OBJECT_INVALID, FX_ID_SHIELD_SONIC, FALSE);
            ...
            //Apply the VFX impact and effects
            gsSPApplyEffect(oPC, TagEffect(eLink, FX_ID_SHIELD_SONIC), SPELL_INVALID, TurnsToSeconds(iDuracionRestringida));
